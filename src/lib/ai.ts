...
function evaluateHeuristic(state: GameState, aiPlayer: Player): number {
  const { boards } = state;
  let score = 0;
...
function getTwoInARowCells(cells: (Player | null)[], boardIndex: number, player: Player): number[] {
  const boardCells = getBoardCells(boardIndex).map(i => cells[i]);
  const twoInARow: number[] = [];
...
function sortMovesByPreference(moves: { boardIndex: number; cellIndex: number }[], state: GameState): { boardIndex: number; cellIndex: number }[] {
...
function getSimpleAIMove(state: GameState): { boardIndex: number; cellIndex: number } | null {
  const moves = getValidMoves(state);
...
function getMedianAIMove(state: GameState): { boardIndex: number; cellIndex: number } | null {
  const moves = getValidMoves(state);
  if (moves.length === 0) return null;
...
function getHardAIMove(state: GameState): { boardIndex: number; cellIndex: number } | null {
  const moves = getValidMoves(state);
  if (moves.length === 0) return null;
